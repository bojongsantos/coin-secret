...
export function AccountModule({ user }: { user: CurrentUserDto }) {
  ...
  return (
    <div className="mx-auto flex max-w-3xl flex-col gap-5 p-6">
      ...
      <section className="card p-5"><div className="flex items-center gap-2"><Crown className="size-5 text-warning" /><h2 className="text-sm font-bold">Coin Secret Pro</h2></div><p className="mt-2 text-sm text-muted">Seluruh coin dan seluruh trading plan, tanpa batas pencarian.</p><p className="mt-4 text-2xl font-bold">{formatUsd(billingPlan("annual").perMonthUsd)} <span className="text-xs font-normal text-muted">/ bulan pada paket tahunan</span></p>{user.plan === "PREMIUM" ? <p className="mt-4 text-sm font-semibold text-positive">Pro aktif.</p> : <Link href="/pricing" className="mt-4 inline-flex items-center gap-2 rounded-lg bg-gradient-to-r from-accent to-accent-blue px-4 py-2.5 text-sm font-bold text-white">Lihat paket</Link>}</section>
