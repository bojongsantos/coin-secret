  ...
  if (data.available === false) {
    return (
      <section className="cs-card p-5">
        <h3 className="text-[16px] font-semibold">{t("rail.marketSentiment")}</h3>
  ...
  return (
    <section className="cs-card p-5">
      ...
      <div className="mt-4 flex justify-center">
        <Gauge score={data.score} label={data.label} width={310} />
      </div>
      ...
      <div className="mt-5">
        ...
        <div className="mt-3 flex flex-wrap justify-between gap-x-2 gap-y-1">
          {data.distribution.map((zone) => (
            <span key={zone.label} className="inline-flex items-center gap-1.5 text-[11px] text-muted-2">
              <span className="size-2 rounded-full" style={{ backgroundColor: zoneColors[zone.label] }} />
              {zone.label}
            </span>
          ))}
        </div>
