...
import { lockedSetupOutcome } from "@/core/domain/analysis/supply-demand";
...
test("a setup still between its levels stays running", () => {
  ...
  assert.equal(lockedSetupOutcome(candles, LONG, 105), null);
});
...
test("a wick through the stop ends the setup even after price recovers", () => {
  ...
  assert.equal(lockedSetupOutcome(candles, LONG, 104), "stopped");
});
...
test("a spike through the second target counts even if price retraces", () => {
  ...
  assert.equal(lockedSetupOutcome(candles, LONG, 110), "target");
});
...
test("the stop wins when both levels were touched in the same window", () => {
  ...
  assert.equal(lockedSetupOutcome(candles, LONG, 110), "stopped");
});
...
test("bars from before the lock are ignored", () => {
  ...
  assert.equal(lockedSetupOutcome(earlier, LONG, 104), null);
});
...
test("the spot price still decides when no bar has closed yet", () => {
  ...
  // there is to go on.
  assert.equal(lockedSetupOutcome([], LONG, 94), "stopped");
  assert.equal(lockedSetupOutcome([], LONG, 121), "target");
  assert.equal(lockedSetupOutcome([], LONG, 110), null);
});

test("a short is decided by the mirrored levels", () => {
  const stopped = bars([[106, 99, 101]]);
  assert.equal(lockedSetupOutcome(stopped, SHORT, 101), "stopped");

  const hit = bars([[101, 79, 82]]);
  assert.equal(lockedSetupOutcome(hit, SHORT, 82), "target");

  const running = bars([[102, 96, 98]]);
  assert.equal(lockedSetupOutcome(running, SHORT, 98), null);
});
